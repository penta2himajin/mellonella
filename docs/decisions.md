...
3. **Extending AS-Norm to other scenarios** (scenario_1 / 4 / 6) — plumbing landed (PR #46); baseline-capture path landed; activation pending one workflow_dispatch run. The runners that build the pipeline (`bench/mellonella_bench/runners/run_all.py` and `scripts/ci_accuracy.py`) now both accept `--as-norm-cohort PATH`, which routes through `RealPipelineProvider`'s existing `as_norm_cohort_path` to flip `GatingConfig.use_as_norm = True` with the data-driven θ defaults from step 1. Scenario modules themselves needed no change — they already consume an opaque `PipelineProvider`. `ci_accuracy.py` reads / writes a separate baseline `docs/benchmarks/ci_baseline_as_norm.json` when `--as-norm-cohort` is set so the legacy `ci_baseline.json` (mixed cosine+pitch scale) and the AS-Norm baseline (z-score scale) do not cross-contaminate. `accuracy.yml` now exposes a `workflow_dispatch` input `capture_as_norm_baseline` that restores the v10 cohort cache from scenario_5, runs `ci_accuracy.py --as-norm-cohort … --update-baseline`, and uploads the resulting `ci_baseline_as_norm.json` as an artifact. The remaining activation step is: trigger that workflow_dispatch once, commit the captured `ci_baseline_as_norm.json` into the repo, then add an AS-Norm leg to the default ci-accuracy job so the regression check exercises both paths on every PR. scenario_4 / scenario_6 currently receive `items=[]` in `run_all.py`, so the flag plumbs through but has no observable effect on those scenarios until item discovery is wired.
