...
6. **Phase 6** (in progress): cohort scale-up. **Part 1** (PR #40, merged): scale per-language 8 → 15, top-K stays at 10 in code (`DEFAULT_TOP_SPEAKERS = 18` in mls/emilia.prepare; `--per-language 15` in scenario_5.yml; `as_norm_top_k = 10` in GatingConfig; cache key v5 → v8 — v6 attempted top_speakers = 60 which CI failed on MLS de test = 30 spk; v7 attempted top_speakers = 30 which CI failed on MLS fr test = 18 spk; v8 saturates the smallest MLS test split at 18 spk). **Part 1.5** (this PR): MLS train-split sourcing for cohort use only — train has 4 500+ spk/lang and is split-disjoint from test by canonical LibriSpeech construction, so the cohort stays guaranteed-disjoint from scenario_5's target / other. mls.py now exposes `--max-stream` / `--max-speakers-seen` CLI flags so the train-split call can bound memory at ≤ 1 GB during the streaming scan; emilia.py bumps `DEFAULT_TOP_SPEAKERS` 18 → 52 (target / other top-2 + cohort top-50 share one manifest); scenario_5.yml runs MLS test prep AND a parallel MLS train prep (output: `data/mls_cohort/<lang>/`) and points the cohort builder at the train manifest for de / fr; cohort `--per-language 15 → 50`; `as_norm_top_k 10 → 20`; cache key v8 → v9. Total cohort = 6 langs × 50 spk = 300 embeddings (vs 90 in v8), reaching the literature 50-100 spk/lang lower bound. **Part 2** (gated on hosted-runner restoration): `theta_pass_as_norm` data-driven re-calibration on the v9 cohort, scenario_5 threshold tightening (combined with Phase 5 closeout follow-up), extending AS-Norm to scenarios 1 / 4 / 6. See "Phase 6: cohort scale-up" below.
...
**Part 1.5 (v9, this PR) — break past the MLS test split via train-split sourcing**:

MLS train has 4 500+ speakers per language and is split-disjoint from test by canonical LibriSpeech construction (each (reader_id, book_id) pair appears in exactly one of train / dev / test). So routing the cohort through MLS train guarantees disjointness from scenario_5's target / other (which still come from MLS test) without needing the `--skip-top-n` carve-out — we still pass `--skip-top-n 2` to the cohort builder uniformly to avoid per-manifest skip-top-n config; the cost is dropping the lex-first 2 train speakers per language, which is harmless.
...
| de / fr cohort | MLS train | `data/mls_cohort/<lang>/` | 52 | build_impostor_cohort.py |
...
For Emilia-YODAS the same 52-speaker manifest serves both roles (no train / test split exists upstream — Emilia is one large speech corpus, so cohort-disjoint is enforced via `--skip-top-n 2` inside the shared manifest). For MLS the dual manifests are needed because the test split is too small (18 spk) to support both top-2 (target / other) and top-50 (cohort) from a single manifest.

**Code changes (this PR — Part 1.5, v9)**:

- `bench/mellonella_bench/datasets/mls.py`: add `max_speakers_seen: int | None = None` parameter to `prepare()` — when set, the streaming loop refuses to register new speakers once the bucket reaches that size, while existing speakers can still grow to `per_speaker_cap`. This is the memory bound the train-split path needs (≤ 60 × 16 clips × ~960 KB / clip ≈ 0.92 GB peak); the test-split path leaves it None (no cap, current behavior). CLI exposes new `--max-stream` and `--max-speakers-seen` flags so the train invocation can pass `--max-stream 30000 --max-speakers-seen 60 --top-speakers 52`.
- `bench/mellonella_bench/datasets/emilia.py`: bump `DEFAULT_TOP_SPEAKERS` 18 → 52. Emilia shard speaker density is high (hundreds per shard); the existing 5 000-sample scan window comfortably surfaces 52 speakers. No max_speakers_seen cap needed (peak ≈ 0.5 GB).
- `.github/workflows/scenario_5.yml`: add a new step "Prepare MLS train-split subsets (de, fr — AS-Norm cohort)" that runs `mls.py prepare --split train --top-speakers 52 --max-stream 30000 --max-speakers-seen 60 --data-dir $MELLONELLA_DATA_DIR/mls_cohort` for de and fr. Reroute the cohort-build step's `--manifest de=...` / `--manifest fr=...` from `data/mls/<lang>/manifest.csv` to `data/mls_cohort/<lang>/manifest.csv`. Bump `--per-language 15 → 50`. Bump cache key v8 → v9 (manifest layout changes — the v8 artifact has no `mls_cohort/` tree so it cannot be reused). Bump `timeout-minutes 45 → 60` to budget for the new train-prep step. The scenario_5 run step itself is unchanged (still consumes `data/mls/<lang>/manifest.csv` for de / fr target / other).
- `poc/mellonella_poc/config.py`: bump `as_norm_top_k` 10 → 20. New cohort = 300 embeddings; top-K 20 = 6.7 % of cohort (within the literature 5-30 % range, lower than v8's 11 %). Top-K 20 is also the literature midpoint of 10-50, where μ/σ estimation noise stops dominating the centering.

**Cost projection (v9)**:

- Manifest prep (cold cache): MLS test path unchanged (~5 min). MLS train path is the new addition — each language scans up to 30 000 samples; HF parquet streaming amortises HTTP per shard, so 30 000 metadata-only-ish samples per language ≈ 5-10 min. Two languages ≈ 10-20 min. Emilia path scaled 18 → 52 speakers — same scan window but more clips kept; cold-cache time grows ~2× to ~3-5 min per language × 4 languages ≈ 12-20 min. Total cold-cache scenario_5 prep ≈ 25-40 min, comfortably inside the new 60 min job timeout.
...
- Warm cache (steady state once main has cached the v9 artifacts): no measurable change vs v8.
- Cohort `.npz` size: 300 × 192 × 4 B ≈ 230 KB (vs 70 KB for v8). Still negligible for the artifact upload.
- Memory: MLS train prep peak ≤ 1 GB; Emilia prep peak ≤ 0.5 GB; ECAPA model resident ≈ 100 MB. Worst case ≈ 1.5 GB, well within the 7 GB CI runner budget.

**Risk: train-split clip density per speaker is the unknown**

We don't have direct measurements of MLS train shard structure under `--max-stream 30000`. If clip density is much lower than expected (e.g. 50 clips / speaker average instead of 600+), the 30 000-sample window might surface <52 speakers and the prep raises `RuntimeError: MLS(<lang>, train) yielded only N speaker(s) after 30000 samples; need 52`. Mitigation: bump `--max-stream` to 60 000 in a follow-up; the `max_speakers_seen=60` cap keeps memory bounded regardless of stream window size. Same failure-mode-then-bump pattern that v6/v7/v8 used to converge on the test-split bound.

**Deferred to Phase 6 follow-up part 2** (needs CI baseline observation, gated on hosted-runner restoration):

1. **`theta_pass_as_norm` data-driven re-calibration**. The heuristic 1.5 σ default was empirically validated on the Phase 4-5 cohort (48 embeddings, top-K 10). After v9 the cohort grows to 300 embeddings with top-K 20 — both the candidate pool size AND the top-K shift, so the impostor-tail distribution moves materially. Re-running `scripts/calibrate.py --use-as-norm --cohort data/cohorts/scenario5_cohort.npz` on the v9 cohort is required before the heuristic can be replaced with a fitted value. Same deferral pattern as Phase 3 → Phase 5 closeout: heuristic shipped, data-driven update gated on one stable baseline observation.
2. **Threshold tightening** (`scenario_5.yml --tpr-min` / `--fpr-max`). Inherits the Phase 5 closeout deferral. Once the v9 cache is populated and we have one bit-identical CI baseline, both Phase 5 and Phase 6 follow-ups can land in the same PR.
...
- Once a Phase 6 push-event run completes successfully, observe the per-row TPR / FPR baseline with the v9 cohort.
- Run the same commit twice; bit-identical guarantee should still hold (Phase 5 fixes apply unchanged; the train-split scan is also deterministic given DATASET_REVISION + parquet shard order).
- Run `scripts/calibrate.py --use-as-norm --cohort data/cohorts/scenario5_cohort.npz` against the v9 cohort, propagate the recommended `theta_pass_as_norm` into `GatingConfig` default.
