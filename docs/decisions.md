...
**Step 3: zh-CN per-language θ_pass_as_norm investigation** (PR-A this PR — `scripts/calibrate_per_language.py` + `scenario_5.yml` workflow_dispatch hook).

The PR #54 scenario_6 baseline put concrete numbers on the zh-CN outlier story: at the global θ_pass_as_norm = 2.25, scenario_6 saw zh-CN gate_tpr 0.317 / auto_learn_admissions 0 / anchor_distance_final missing, vs gate_tpr ≥ 0.93 / admissions 6-50 for all 5 other languages. Combined with the Phase 6 Part 2 step 2 closeout's scenario_5 FPR-mean spread (zh-CN 0.084-0.184 vs ≤0.062 for the rest), this is enough evidence that the global θ — anchored on librosa libri1/2/3 (English LibriSpeech) — is mis-serving zh-CN.

PR-A does the **measurement**: a new `scripts/calibrate_per_language.py` reuses `calibrate.py`'s `measure_cells` / `aggregate` / `recommend_theta` per language, picking the top-3 speakers from each prepared MLS / Emilia manifest (matches the libri1/2/3 cell count = 108 cells/lang × 6 languages = 648 cells × ~3.5 s/cell ≈ ~38 min on a warm cohort cache). Output is one combined `calibration_as_norm_per_language.json` with per-language `recommended_theta_pass`, the per-θ aggregate, and a `spread_analysis` block (range_width vs `decision_threshold = 0.5` → adopt per-language table / keep global). Wired as a `workflow_dispatch` input on `scenario_5.yml` (`capture_per_language_theta`); not run on per-PR pushes (cost-prohibitive). Pattern mirrors the AS-Norm baseline-capture workflow_dispatch in `accuracy.yml`.

PR-B (follow-up) will read the resulting summary and decide:
- range_width ≤ 0.5 → keep global θ = 2.25 (record per-language values + decision in decisions.md, no propagation needed).
- range_width > 0.5 → adopt `GatingConfig.theta_pass_as_norm: dict[str, float] | float`, propagate per-language values, and re-capture `ci_baseline_as_norm.json` (cohort hashing in PR #53's calibrate cache key changes nothing — only the GatingConfig per-language pathway changes how scoring routes, not the cohort itself). The pipeline `_score` codepath then needs a language tag passed through; that's the deeper structural change PR-B has to weigh.
