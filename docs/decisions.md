...
1. **`theta_pass_as_norm` data-driven re-calibration**. The heuristic 1.5 σ default was empirically validated on the Phase 4-5 cohort (48 embeddings, top-K 10). After v10 the cohort grows to 300 embeddings with top-K 20 — both the candidate pool size AND the top-K shift, so the impostor-tail distribution moves materially. Re-running `scripts/calibrate.py --use-as-norm --cohort data/cohorts/scenario5_cohort.npz` on the v10 cohort is required before the heuristic can be replaced with a fitted value. Same deferral pattern as Phase 3 → Phase 5 closeout: heuristic shipped, data-driven update gated on one stable baseline observation.
