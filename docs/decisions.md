...
2. **Threshold tightening** ✅ (Phase 5 closeout + Phase 6 Part 2 step 2 combined). Anchored on three θ=2.25 baselines (main run 25431310778, PR #45 run 25432567972, PR #46 run 25439461512, all 2026-05-06): with `theta_pass_as_norm = 2.25`, scenario_5 mean TPR 0.717–0.748 (was 0.799 at θ=1.5), mean FPR 0.017–0.025 (was 0.062). The legacy θ=1.5 worst-row FPR was 0.712; θ=2.25 collapses the zh-CN outlier ~3×. **Per-row variance was the binding constraint**: cohort SHA256 matches between runs (Phase 5 determinism holds) but ECAPA+DFN3 float32 SIMD on different runner CPU classes shifts the worst rows materially. Observed worst-row range across the three samples: zh-CN @ snr=0 target TPR `0.140 / 0.276 / 0.376` (range 0.236), zh-CN @ snr=15 other FPR `0.087 / 0.136 / 0.200` (range 0.113) — the third sample widened the variance well beyond what the first two suggested, prompting a follow-up loosening. Final thresholds: **`--tpr-min 0.10`** (worst observed 0.140 − ~30% margin) and **`--fpr-max 0.30`** (worst observed 0.200 + ~50% margin) — still 3× tighter than the legacy 0.95 placeholder while absorbing the full observed range. The `--fpr-max < --tpr-min` invariant (Phase 5 closeout goal) is **not** satisfied here (0.30 > 0.10) and is deferred: closing it requires either pinning the runner to a single CPU class or replacing the per-row check with a per-language-mean check (mean FPR ~0.02 is well below any reasonable tpr_min). Recorded as a Phase 6 part 3 / Phase 7 follow-up.
