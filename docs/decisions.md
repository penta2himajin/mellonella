...
**Step 3: zh-CN per-language θ_pass_as_norm investigation** ✅ (PR-A `scripts/calibrate_per_language.py` + `scenario_5.yml` workflow_dispatch hook; PR-A2 grid extension to 0.0; PR-B closeout — **decision: keep global θ = 2.25**, no propagation).
...
PR-A built the **measurement**: a new `scripts/calibrate_per_language.py` reuses `calibrate.py`'s `measure_cells` / `aggregate` / `recommend_theta` per language, picking the top-3 speakers from each prepared MLS / Emilia manifest (matches the libri1/2/3 cell count = 108 cells/lang × 6 languages = 648 cells, ~12 min on a warm cohort cache after PR #57's `_trim_speakers` 6 s cap). Output: one combined `calibration_as_norm_per_language.json` with per-language `recommended_theta_pass`, per-θ aggregate, and a `spread_analysis` block. Wired as a `workflow_dispatch` input on `scenario_5.yml` (`capture_per_language_theta`); not run on per-PR pushes. PR-A2 (THETA_GRID_AS_NORM extension 0.5-3.0 → 0.0-3.0) was needed because the first sweep had 4/6 recommendations clamped to the 0.5 grid floor.

**Final per-language recommendations** (run 25494354564, commit 47e8c5b7, archived as `docs/benchmarks/calibration_as_norm_per_language.json`):

| lang | rec θ | meaningful? |
|------|-------|-------------|
| de | 1.0 | ✅ FPR 0.099 → 0.108 across 1.0 ↔ 0.5 (genuine middle-of-grid pick) |
| zh-CN | 1.5 | ✅ FPR 0.097 (just under the 0.10 cap); falls below cap only at θ ≥ 1.5 |
| en | 0.25 | ✅ on the FPR cliff (FPR 0.46 at θ=0.0 → 0.011 at θ=0.25); θ=0.25 = optimal |
| fr | 0.25 | ✅ same cliff pattern (FPR 0.54 → 0.029) |
| ko | 0.25 | ✅ same cliff pattern (FPR 0.53 → 0.036) |
| ja | 0.25 | ⚠️ `recommend_theta` fallback path — TPR_median never reaches the 0.5 floor at any θ; 0.25 is "smallest θ meeting the FPR budget alone". The cohort-distance issue for ja is not solvable by θ tuning. |

range_width = 1.25 (zh-CN 1.5 − en/fr/ja/ko 0.25); the spread_analysis emits `decision: adopt_per_language_table`. **PR-B nevertheless decides to keep the global θ = 2.25**, on the following grounds:

1. **Modest expected TPR uplift**. Across non-zh-CN languages, the calibrate per-language `tpr_median` differs by only +0.05 to +0.10 between θ=2.25 and θ=0.25/1.0. The current scenario_5 numbers under θ=2.25 already pass the per-language-mean check (TPR ≥ 0.40, FPR ≤ 0.30) for all 6 languages with 25-63% margin.
2. **zh-CN scenario_5 FPR is the binding constraint, not the calibrate FPR**. At θ=2.25, calibrate-FPR for zh-CN is 0.033 but scenario_5 actual is 0.133 — a ~4× scaling. Linearly extrapolating, dropping the global θ to 1.5 would push zh-CN scenario_5 FPR to ~0.39, exceeding the 0.30 per-language-mean ceiling. A 2-tier table (`{zh-CN: 2.25, others: 1.5}`) avoids this but requires `GatingConfig.theta_pass_as_norm: dict[str, float] | float`, a language tag flowing into the pipeline `_score` path, and re-capture of `ci_baseline_as_norm.json`.
3. **ja's TPR ceiling is independent of θ**. ja's TPR_median caps at ~0.40 even at θ=0.0 — a cohort-distance problem, not a threshold-tuning problem. No θ choice fixes ja; that requires either expanding the cohort (Phase 6 part 1.5 already pushed it from 90 → 300 embeddings) or moving to a different normalization (out of scope here).
4. **The investigation itself is the deliverable**. The libri1/2/3-anchored global θ = 2.25 turned out to be conservative for 5 of 6 languages, but adapting it carries structural cost — `GatingConfig` schema change, pipeline language-tagging, baseline re-captures — that the modest uplift does not pay for. The data is archived (`docs/benchmarks/calibration_as_norm_per_language.json`) so a future Rust-port re-architecture or cohort-redesign can revisit cleanly.

scripts/calibrate_per_language.py and the workflow_dispatch hook stay in main for future re-evaluation when the cohort or pipeline changes.
