...
2. **Threshold tightening** ✅ (Phase 5 closeout + Phase 6 Part 2 step 2 combined). Anchored on the first post-#44 main baseline (run 25431310778, 2026-05-06): with `theta_pass_as_norm = 2.25`, scenario_5 observed TPR mean 0.730 (was 0.799 at θ=1.5), FPR mean 0.023 (was 0.062). Per-row worst: TPR 0.276 (zh-CN @ snr=0), FPR 0.136 (zh-CN @ snr=15) — the legacy θ=1.5 worst-row FPR was 0.712, so θ=2.25 collapses the zh-CN outlier ~5x. New thresholds: **`--tpr-min 0.20`** (worst row + 0.076 margin; floor moves DOWN from 0.30 because the stricter pass gate naturally drops at-edge TPR) and **`--fpr-max 0.18`** (worst row + 0.044 margin; ceiling moves DOWN ~5x from 0.95). The `--fpr-max < --tpr-min` invariant is now satisfied (0.18 < 0.20).
