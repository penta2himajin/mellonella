...
**Phase 6 follow-up part 2** (hosted-runner restored as of 2026-05-06):

1. **`theta_pass_as_norm` data-driven re-calibration** ✅ (PR #43 + propagation PR). PR #43 wired `scripts/calibrate.py --use-as-norm --cohort data/cohorts/scenario5_cohort.npz` into `scenario_5.yml` after the cohort build, staging `calibration_as_norm_summary.json` into the `scenario-5-results` artifact. The first run on the v10 cohort (300 embeddings, 6 langs × 50 spk, top-K=20) recommended **θ_pass = 2.25** (per-θ sweep: TPR mean 0.755 / FPR mean 0.085 — smallest θ within `MAX_MEAN_FPR_AS_NORM = 0.10`). The follow-up PR propagated `GatingConfig.theta_pass_as_norm: 1.5 → 2.25` and `theta_learn_as_norm: 2.5 → 3.25` (preserving the +1.0σ heuristic gap; `theta_learn` remains heuristic since `calibrate.py`'s grid maxes at 3.0). The previous 1.5σ default was anchored on the Phase 4-5 cohort (48 embeddings, top-K=10) — scaling cohort + top-K shifts the impostor-tail distribution and therefore the optimal θ.
2. **Threshold tightening** ✅ (Phase 5 closeout + Phase 6 Part 2 step 2 combined, ultimately closed out by switching to a per-language-mean check). Anchored on four θ=2.25 baselines (main run 25431310778, PR #45 run 25432567972, PR #46 run 25439461512, PR #49 run 25443974256, all 2026-05-06): with `theta_pass_as_norm = 2.25`, scenario_5 mean TPR 0.717–0.748 (was 0.799 at θ=1.5), mean FPR 0.017–0.062 (was 0.062). The legacy θ=1.5 worst-row FPR was 0.712; θ=2.25 collapsed the zh-CN outlier ~3× *on the central observations*, but the fourth sample surfaced a worst-row FPR of 0.400 for the same `zh-CN_speaker02_vs_speaker20` sample, widening the per-row range to 0.087–0.400. **Per-row variance is structurally too large for hard-fail thresholds**: cohort SHA256 matches between runs (Phase 5 determinism holds) but ECAPA+DFN3 float32 SIMD on different runner CPU classes shifts the per-row decisions enough that any per-row ceiling either flakes (tight) or catches nothing (loose). The check was retired and replaced with a **per-language-mean check** (`--tpr-mean-min` / `--fpr-mean-max` against `gate_tpr_mean__<lang>` / `gate_fpr_mean__<lang>`). Per-language ranges across the four runs: zh-CN TPR mean 0.503–0.659 / FPR mean 0.084–0.184; ko TPR 0.692–0.782; en TPR 0.623–0.700; the rest are tighter. New thresholds: **`--tpr-mean-min 0.40`** (worst observed 0.503 has ~25% margin) and **`--fpr-mean-max 0.30`** (worst observed 0.184 has ~63% margin). The `--fpr-mean-max < --tpr-mean-min` invariant (the Phase 5 closeout goal) is **finally satisfied**: 0.30 < 0.40.
3. **Extending AS-Norm to other scenarios** ✅ (scenario_1 activated end-to-end; scenario_4 / 6 plumbed but inactive). The runners that build the pipeline (`bench/mellonella_bench/runners/run_all.py` and `scripts/ci_accuracy.py`) both accept `--as-norm-cohort PATH`, which routes through `RealPipelineProvider`'s existing `as_norm_cohort_path` to flip `GatingConfig.use_as_norm = True` with the data-driven θ defaults from step 1. Scenario modules themselves needed no change — they already consume an opaque `PipelineProvider`. `ci_accuracy.py` reads / writes a separate baseline `docs/benchmarks/ci_baseline_as_norm.json` when `--as-norm-cohort` is set so the legacy `ci_baseline.json` (mixed cosine+pitch scale) and the AS-Norm baseline (z-score scale) do not cross-contaminate. `accuracy.yml` exposes a `workflow_dispatch` input `capture_as_norm_baseline` that captures / refreshes the AS-Norm baseline (run 25481044895 produced the initial values; per-SNR TPR 0.290 / 0.429 / 0.762 at SNR 5/10/15 dB, FPR 0 across the board, sim4 TPR 0.463 / 0.147 at +9 / 0 dB). The default ci-accuracy run on push / PR now executes both legs in parallel — the legacy mixed-score job against `ci_baseline.json` and a new `ci-accuracy AS-Norm` job that fetches the cohort from scenario_5's latest successful main artifact (same `gh run download` pattern as the workflow_dispatch capture job) and compares against `ci_baseline_as_norm.json`. scenario_4 / scenario_6 currently receive `items=[]` in `run_all.py`, so the flag plumbs through but has no observable effect on those scenarios until item discovery is wired.
...
**Step 2: scenario_6 (drift verification) wiring** (this PR — `scripts/scenario_6_from_manifest.py` + `.github/workflows/scenario_6.yml`).

Mirrors the scenario_4 driver / workflow structure. Reuses the existing MLS de+fr + Emilia-YODAS 6-language manifests at the **clip-level** granularity (`read_manifest()`) instead of the per-speaker concatenated form scenario_4 / scenario_5 use. Per language: rank speakers by total clip duration, pick the top speaker, sort their clips lex by `clip_path` deterministically, and emit `Scenario6Item(enrollment_path=clip[0], variant_paths=clip[1..variants_per_speaker])` (default `variants_per_speaker = 3`). The runner concatenates enrollment + variants into one buffer and runs the gate end-to-end so auto-learn admission / reset behaviour is exercised on the real pipeline.

Drift signal is intentionally thin in this PR: dataset prep currently emits `clips_per_speaker = 4` (= 1 enrollment + 3 variants ≈ 40 s tracks). Concretely this is a *long-clip stability test* rather than a multi-session drift test — adequate for the PoC closeout deliverable (auto-learn pool connected end-to-end, admission counters / `anchor_distance_final` come back finite from `RealPipelineProvider`). If the observed metrics are uninformative (e.g. zero admissions on every language, indistinguishable across languages), a follow-up PR will bump `clips_per_speaker` to 8-12 and re-capture the AS-Norm baseline + workflow_dispatch baseline-capture artefact (`ci_baseline_as_norm.json`) since cohort `.npz` contents shift when per-speaker concat lengths change. The cascade cost is bounded (~30 min one-time cold-cache prep + ~6 min calibrate re-run cached after PR #53 + workflow_dispatch baseline re-capture). Observation-only thresholds: no `failures.json` until two stable main runs land.

**Step 3: zh-CN per-language θ_pass_as_norm investigation** ✅ (PR-A `scripts/calibrate_per_language.py` + `scenario_5.yml` workflow_dispatch hook; PR-A2 grid extension to 0.0; PR-B closeout — **decision: keep global θ = 2.25**, no propagation; PR-A3 / PR-B2 follow-up — calibrate methodology fix: 6 s test audio was an artifact, default bumped to 16 s and per-language data re-baselined).

The PR #54 scenario_6 baseline put concrete numbers on the zh-CN outlier story: at the global θ_pass_as_norm = 2.25, scenario_6 saw zh-CN gate_tpr 0.317 / auto_learn_admissions 0 / anchor_distance_final missing, vs gate_tpr ≥ 0.93 / admissions 6-50 for all 5 other languages. Combined with the Phase 6 Part 2 step 2 closeout's scenario_5 FPR-mean spread (zh-CN 0.084-0.184 vs ≤0.062 for the rest), this looked like enough evidence that the global θ — anchored on librosa libri1/2/3 (English LibriSpeech) — was mis-serving zh-CN.

PR-A built the **measurement**: a new `scripts/calibrate_per_language.py` reuses `calibrate.py`'s `measure_cells` / `aggregate` / `recommend_theta` per language, picking the top-3 speakers from each prepared MLS / Emilia manifest (matches the libri1/2/3 cell count = 108 cells/lang × 6 languages = 648 cells). Output: one combined `calibration_as_norm_per_language.json` with per-language `recommended_theta_pass`, per-θ aggregate, and a `spread_analysis` block. Wired as a `workflow_dispatch` input on `scenario_5.yml` (`capture_per_language_theta`); not run on per-PR pushes. PR-A2 (THETA_GRID_AS_NORM extension 0.5-3.0 → 0.0-3.0) was needed because the first sweep had 4/6 recommendations clamped to the 0.5 grid floor.

**Methodology fix (PR-A3 follow-up — `--max-seconds-per-speaker` default 6 s → 16 s)**. The first 6 s setting (matching libri1/2/3's typical clip length) turned out to be an audio-length artifact: at 6 s, ja's `tpr_median` floor-clamped at 0.18-0.40 across the entire θ grid and `recommend_theta` fell into its fallback path (smallest θ meeting just the FPR budget). Two diagnostic workflow_dispatch runs ruled this in step by step: a white-only sweep falsified the pink-noise hypothesis (run 25496800095 — ja TPR_median 0.371 white-only vs 0.398 white+pink at θ=0.25, no improvement); a 16 s rerun (run 25498662712, archived as the JSON below) showed ja's `tpr_median` jumping to 0.728 at θ=2.25 and 0.822 at θ=0.25, matching scenario_5's observed ja TPR mean of 0.852. Conclusion: 6 s test audio (= 3 s post half-split) was insufficient for ECAPA's statistics-pooling layer to converge on non-libri1/2/3 languages, and 16 s aligns the calibrate footprint with scenario_5's `--max-seconds-per-speaker 8.0` test audio.

**Final per-language recommendations** (run 25498662712, commit 622c6068, 16 s audio, archived as `docs/benchmarks/calibration_as_norm_per_language.json`):

| lang | rec θ | TPR_med @ rec θ | FPR_mean @ rec θ | meaningful? |
|------|-------|-----------------|------------------|-------------|
| de | 2.25 | 0.674 | 0.089 | ✅ matches the current global; FPR 0.090 just under the 0.10 cap |
| fr | 0.5 | 0.760 | 0.099 | ✅ FPR cliff (0.355 at θ=0.0 → 0.099 at 0.5) |
| en | 0.25 | 0.836 | 0.069 | ✅ FPR cliff (0.267 → 0.069) |
| ja | 0.25 | 0.822 | 0.055 | ✅ **genuine pick** (no longer fallback — the 6 s data was misleading) |
| ko | 0.75 | 0.774 | 0.077 | ✅ FPR cliff (0.388 at θ=0.0 → 0.077 at 0.75) |
| zh-CN | 3.0 | 0.608 | 0.180 | ⚠️ `recommend_theta` 3rd fallback — `fpr_mean ≤ 0.10` is unreachable in the entire grid (min observed FPR is 0.180 at θ=3.0). True cohort-distance problem, not θ-solvable |

range_width = 2.75 (zh-CN 3.0 − en/ja 0.25). The spread_analysis still emits `decision: adopt_per_language_table`, but **PR-B's closeout decision to keep the global θ = 2.25 stays** — the 16 s data sharpens the picture without changing the conclusion:

1. **5/6 languages would gain modest TPR uplift** (+0.024 to +0.094 `tpr_median` going from 2.25 → per-language θ). Sizable for ja (+0.094) and ko (+0.076), small for fr (+0.024). de matches global already so no change.
2. **zh-CN now looks more outlier than the 6 s data suggested**: at θ=2.25 with 16 s, calibrate-FPR for zh-CN is 0.201 (vs 0.033 at 6 s) — much closer to scenario_5's actual 0.133. Linearly extrapolating, dropping the global θ to 1.5 would push zh-CN scenario_5 FPR to ~0.55, far exceeding the 0.30 per-language-mean ceiling. A 2-tier table (`{zh-CN: 2.25, others: per-lang}`) is the only safe option, which still requires `GatingConfig.theta_pass_as_norm: dict[str, float] | float`, a language tag flowing into the pipeline `_score` path, and re-capture of `ci_baseline_as_norm.json`. The structural cost did not pay for the +0.024-0.094 uplift before, and still does not now.
3. **ja's fallback was a methodology artifact, not a cohort-distance problem**. The 16 s rerun resolved it cleanly (TPR_median 0.728 at θ=2.25). The deeper takeaway: calibrate's libri1/2/3 anchor was hiding a real production-vs-calibrate audio-length mismatch for half the language matrix.
4. **zh-CN's true cohort-distance issue surfaces clearly at 16 s**. Even at the strictest grid θ=3.0, FPR stays at 0.18 — well above the 0.10 cap. Fixing zh-CN requires either a larger / more diverse zh-CN cohort partition or a different normalization (out of scope here).

scripts/calibrate_per_language.py and the workflow_dispatch hook stay in main for future re-evaluation when the cohort or pipeline changes (notably the implementation.md Phase 3 Rust port, where the pipeline `_score` codepath is being rewritten anyway and the per-language dict could be added at low marginal cost).
