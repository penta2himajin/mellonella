...
3. **Extending AS-Norm to other scenarios** (scenario_1 / 4 / 6) — plumbing landed; CI activation deferred. The runners that build the pipeline (`bench/mellonella_bench/runners/run_all.py` and `scripts/ci_accuracy.py`) now both accept `--as-norm-cohort PATH`, which routes through `RealPipelineProvider`'s existing `as_norm_cohort_path` to flip `GatingConfig.use_as_norm = True` with the data-driven θ defaults from step 1. Scenario modules themselves needed no change — they already consume an opaque `PipelineProvider`. **Activation** (wiring the v10 cohort into `accuracy.yml` so ci-accuracy actually exercises the AS-Norm path on every PR) is held back as a separate follow-up because the legacy `docs/benchmarks/ci_baseline.json` was captured on the mixed-score path; flipping ci-accuracy to AS-Norm without re-baselining would hard-fail the regression check on every key. The clean sequence is: land plumbing → run accuracy.yml once with `--as-norm-cohort … --update-baseline` → commit the new `ci_baseline.json` → land activation. scenario_4 / scenario_6 currently receive `items=[]` in `run_all.py`, so the flag plumbs through but has no observable effect on those scenarios until item discovery is wired.
