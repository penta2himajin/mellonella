...
2. **Threshold tightening** ✅ (Phase 5 closeout + Phase 6 Part 2 step 2 combined). Anchored on two θ=2.25 baselines (main run 25431310778, PR #45 run 25432567972, both 2026-05-06): with `theta_pass_as_norm = 2.25`, scenario_5 mean TPR 0.730–0.748 (was 0.799 at θ=1.5), mean FPR 0.017–0.023 (was 0.062). The legacy θ=1.5 worst-row FPR was 0.712; θ=2.25 collapses the zh-CN outlier ~3×. **Per-row variance was the binding constraint**: cohort SHA256 matches between runs (Phase 5 determinism holds) but ECAPA+DFN3 float32 SIMD on different runner CPU classes shifts the worst rows by ±0.03 FPR / ±0.05 TPR — the `zh-CN @ snr=15` other-mode FPR was 0.136 on one runner and 0.200 on another for the identical sample. New thresholds: **`--tpr-min 0.20`** (~2.5× variance margin below the observed worst 0.276) and **`--fpr-max 0.30`** (~3× variance margin above the observed worst 0.200) — 3× tighter than the legacy 0.95 placeholder while avoiding flake. The `--fpr-max < --tpr-min` invariant (Phase 5 closeout goal) is **not** satisfied here (0.30 > 0.20) and is deferred: closing it requires either pinning the runner to a single CPU class or replacing the per-row check with a per-language-mean check (mean FPR ~0.02 is well below any reasonable tpr_min). Recorded as a Phase 6 part 3 / Phase 7 follow-up.
