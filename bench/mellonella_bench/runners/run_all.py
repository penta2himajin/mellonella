...
def run(config: RunnerConfig) -> RunSummary:
    ...
    if "scenario_2" in config.scenarios:
        from ..scenarios.scenario_2 import run as run_scenario_2

        result = run_scenario_2(
            items=[],
            provider=provider,
            sample_rate=16_000,
            output_csv=config.output_dir / "scenario_2.csv",
        )
        summary.scenarios["scenario_2"] = {
            "n_samples": result.n_samples,
            "metrics": result.metrics,
        }
    ...
    if "scenario_5" in config.scenarios:
        from ..scenarios.scenario_5 import run as run_scenario_5

        result = run_scenario_5(
            items=[],
            provider=provider,
            sample_rate=16_000,
            output_csv=config.output_dir / "scenario_5.csv",
        )
        summary.scenarios["scenario_5"] = {
            "n_samples": result.n_samples,
            "metrics": result.metrics,
        }

    if "scenario_6" in config.scenarios:
        from ..scenarios.scenario_6 import run as run_scenario_6

        result = run_scenario_6(
            items=[],
            provider=provider,
            sample_rate=16_000,
            output_csv=config.output_dir / "scenario_6.csv",
        )
        summary.scenarios["scenario_6"] = {
            "n_samples": result.n_samples,
            "metrics": result.metrics,
        }
