...
/// HuggingFace repo hosting the mellonella-side ONNX export of DFN3.
/// DFN3 ships as a torch checkpoint via `df.enhance`; the streaming
/// per-frame ONNX in this repo is generated by
/// `scripts/export_dfn3_onnx.py` and is **not** interchangeable with
/// other community DFN3 ONNX mirrors (it exposes the GRU states as
/// explicit inputs/outputs so the Rust pipeline can drive it one
/// hop at a time).
pub const DFN3_REPO: &str = "penta2himajin/deepfilternet3-onnx";
pub const DFN3_FILE: &str = "dfn3.onnx";

/// HuggingFace repo hosting the mellonella-side ONNX export of
/// ECAPA-TDNN. Generated from SpeechBrain's `spkrec-ecapa-voxceleb`
/// by `scripts/export_ecapa_onnx.py`. Input is the SpeechBrain-
/// compatible 80-mel fbank (computed Rust-side); output is the
/// 192-dim speaker embedding. Other community ECAPA ONNX exports
/// typically take raw waveform input and won't drop in here.
